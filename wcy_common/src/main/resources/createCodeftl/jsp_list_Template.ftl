			...
			<div class="row-fluid">
				<div class="row-fluid">
					<form action="${r"${ctx}"}/${objectNameLower}/list.action" method="post" name="Form" id="Form">
						...
						<table>
							<tr>
								<td><span class="input-icon"> <input autocomplete="off" id="nav-search-input" type="text" name="keyword" value="${r"${obj.keyword}"}" placeholder="这里输入关键词" /> <i id="nav-search-icon" class="icon-search"></i> </span></td>
								<td><input class="span10 Wdate" name="beginTime" id="beginTime" value="${r"${obj.beginTime}"}" type="text" onclick="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd 00-00-00',lang:'zh-cn'})" readonly="readonly" style="width: 150px;height: 28px" placeholder="开始时间"/></td>
								<td><input class="span10 Wdate" name="endTime" id="endTime" value="${r"${obj.endTime}"}" type="text" onclick="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd 23:59:59',lang:'zh-cn'})" readonly="readonly" style="width: 150px;height: 28px" placeholder="结束时间"/></td>
									...
									<c:when test="${r"${not empty varlist}"}">
										<c:forEach items="${r"${varlist}"}" var="var" varStatus="vs">
											<tr>
												...
												<td class='center' style="width: 30px;">${r"${(obj.pageIndex-1)*obj.pageSize+vs.count}"}</td>
												<#list fieldList as var>
													<#if (var_index > 0)>
												<td>${r"${"}var.${var[0]}}</td>
													</#if>
						...
						<div class="page-header position-relative">
							<table style="width:100%;">
								<tr>
									...
									<td style="vertical-align:top;"><div class="pagination" style="float: right;padding-top: 0px;margin-top: 0px;">${r"${obj.pageStr}"}</div></td>
								</tr>
							</table>
						</div>
					</form>
			...
			if(str==''){
				...
				bootbox.confirm('确定要删除选中的数据吗?', function(result) {
					if(result) {
						$.ajax({
							type: "POST",
							url: '${r"${ctx}"}/${objectNameLower}/delete.action?tm='+new Date().getTime(),
					    	data: {ids:str},
							dataType:'json',
							cache: false,
							success: function(data){
